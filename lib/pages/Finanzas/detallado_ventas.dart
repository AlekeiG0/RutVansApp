...
class _DetalladoVentasPageState extends State<DetalladoVentasPage> {
  ...
  Future<void> _cargarVentas() async {
    ...
    try {
      final List<Map<String, dynamic>> data =
          await LaravelApiService.getVentasDetalle(
    ...
    pdf.addPage(
      pw.Page(
        build: (context) => pw.Column(
          ...
          children: [
            pw.Text(
              'Reporte Detallado - ${displayFormatter.format(fechaSeleccionada)}',
  ...
  Widget build(BuildContext context) {
    final total = ventas.fold(0.0, (sum, v) {
      final monto = double.tryParse(v['amount'].toString()) ?? 0.0;
      return sum + monto;
    });
...
appBarTitle: 'Detalle de Ventas',
  ...
  Widget _buildVentaItem(Map<String, dynamic> venta) {
    final monto = double.tryParse(venta['amount'].toString()) ?? 0.0;
